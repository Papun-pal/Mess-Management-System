  ...
  return (
    <div className="min-h-[75vh] bg-gray-700 p-6">
      <div className="bg-gray-600 shadow-lg rounded-lg p-8 max-w-5xl mx-auto grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {/* Set Total Weekly Fund Section */}
        <div className="bg-blue-200 p-6 rounded-lg shadow-md">
          <h2 className="text-2xl font-bold text-gray-800 mb-4">Select Month & Week</h2>
          <div className="space-y-4">
            ...
            <input
              ...
              className="w-full p-2 border rounded"
            />
            <button onClick={setTotalWeeklyFund} className="w-full bg-blue-600 text-white py-2 rounded hover:bg-blue-700">
              Set Total Weekly Fund
            </button>
            <button onClick={deleteWeeklyFund} className="w-full bg-red-500 text-white py-2 rounded hover:bg-red-700 mt-2">
              Delete Weekly Fund
            </button>
          </div>
        </div>

        {/* Update User Weekly Funds Section */}
        <div className="bg-green-50 p-6 rounded-lg shadow-md">
          <h2 className="text-2xl font-bold text-green-600 mb-4">Update User Weekly Funds</h2>
          <div className="space-y-4">
            <button onClick={fetchUserWeeklyFunds} className="w-full bg-green-600 text-white py-2 rounded hover:bg-green-700">
              Fetch User Weekly Funds
            </button>
            ...
            {loading ? (
              ...
              fundData.map((fund, index) =>
                fund.userId ? (
                  <div key={fund.userId._id || index} className="border-b pb-4 mb-4">
                    <p><strong>Name:</strong> {fund.userId.username}</p>
                    <p><strong>Email:</strong> {fund.userId.email}</p>
                    <p><strong>Amount:</strong> Rs. {fund.amount}</p>
                    <p><strong>Status:</strong> {fund.status}</p>
                    <div className="mt-2">
                      <input
                        type="number"
                        placeholder="Enter Amount"
                        value={updateData.userId === fund.userId._id ? updateData.amount : ""}
                        onChange={(e) =>
                          setUpdateData({
                            userId: fund.userId._id,
                  ...
                  <div key={index} className="bg-yellow-100 text-yellow-800 p-3 rounded mb-4">
                    ⚠️ Skipping a record with missing user information.
                  </div>
                )
              )
            )}
